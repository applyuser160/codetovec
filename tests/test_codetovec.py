...
def test_execute(mock_model: MagicMock, mock_tokenizer: MagicMock) -> None:
...
def test_cleanse_python_code() -> None:
...
def foo():
    ...
    return a
"""
    clean = cv._cleanse_python_code(
        source, remove_comments=True, remove_docstrings=True, remove_blank_lines=True
    )
